...
csv_proveedores = open(os.path.abspath(csv_path + 'GRUPO proveedores.csv'), 'rb')
reader_proveedores = csv.reader(csv_proveedores, dialect='coma_dialecto')

proveedor_account_ids = models.execute_kw(db, uid, password, 'account.account', 'search', [[['code', '=', "400000"], ['company_id', '=', 3]]])
acreedor_account_ids = models.execute_kw(db, uid, password, 'account.account', 'search', [[['code', '=', "410000"], ['company_id', '=', 3]]])
proveedor_fiscal_position_ids = models.execute_kw(db, uid, password, 'account.fiscal.position', 'search', [[['name','=',"Régimen Nacional"],['company_id','=',3]]])
proveedor_fiscal_position_id = proveedor_fiscal_position_ids[0]

c = 0
for linea_proveedores in reader_proveedores:
    c += 1
    print "GRUPO proveedor linea: ", c
    ...
    proveedor_ids = models.execute_kw(db, uid, password, 'res.partner', 'search', [[['name', '=', nombre], ['company_id', '=', 3]]])
    if proveedor_ids:
        continue
    ...
    proveedor_id = models.execute_kw(db, uid, password, 'res.partner', 'create', [{
                        ...
                        'tz': "Europe/Madrid",
                        'comment': vat[2],
#                         'property_account_payable': acc_id,
#                         'property_account_position': proveedor_fiscal_position_id,
                        }])
