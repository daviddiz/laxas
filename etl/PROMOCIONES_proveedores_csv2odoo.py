...
csv_proveedores = open(os.path.abspath(csv_path + 'PROMOCIONES proveedores.csv'), 'rb')
reader_proveedores = csv.reader(csv_proveedores, dialect='coma_dialecto')

proveedor_account_ids = models.execute_kw(db, uid, password, 'account.account', 'search', [[['code', '=', "400000"], ['company_id', '=', 1]]])
acreedor_account_ids = models.execute_kw(db, uid, password, 'account.account', 'search', [[['code', '=', "410000"], ['company_id', '=', 1]]])
...
for linea_proveedores in reader_proveedores:
    c += 1
    print "PROMOCIONES proveedor linea: ", c
...
#     telefono = linea_proveedores[7]
    ...
    proveedor_ids = models.execute_kw(db, uid, password, 'res.partner', 'search', [[['name', '=', nombre], ['company_id', '=', 1]]])
    if proveedor_ids:
        continue
